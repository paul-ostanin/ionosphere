...
open(unit=11, name='stationary_z-projection_-50.txt')
...
call D.init(z.n - 1)
do i = 1, z.n - 1
	D.d(i) = sI**2 * 3E+17 * Tp.interp(z, m.d(i)) / (nO.interp(z, m.d(i)) * sqrt(Tr.interp(z, m.d(i))))
end do
...
call u.init(z.n)
do i = 1, z.n
	u.d(i) = sI**2 * 3E+17 / (nO.d(i) * sqrt(Tr.d(i))) * (56E-6 + gradTp.d(i)) 
end do
...
!		b.d(1) = 0
	...
	nday = njold
		...
		call nday.print(10)
		do i = 1, z.n
			write(11,*) 100+400/(z.n-1)*(i-1), nday.d(i)
		end do
!		write(11, *)
...
!	print *, coschi
	...
	do i = 1, z.n
		if(coschi > 1E-2) then 
			pcurr.d(i) = p.d(i) * exp(tau0.d(i) * (1-1/coschi))
	       	else 
			pcurr.d(i) = 0
	       	end if
	end do
