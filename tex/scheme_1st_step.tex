...
\bigskip {\bf Схема 4.} (Фактически данная схема аппроксимирует не $\dfrac{\partial}{\partial z} (u_\varphi n)$, а $u_\varphi\dfrac{\partial}{\partial z} n$.)
...
\bigskip {\bf Схема 5.} (Фактически данная схема аппроксимирует не $\dfrac{\partial}{\partial z} (u_\varphi n)$, а $u_\varphi\dfrac{\partial}{\partial z} n$.)
...
\bigskip {\bf Схема 6.} Используем схему Куранта-Изаксона-Риис (КИР) для консервативной аппроксимации. Схему запишем исходя из аппроксимации потока для слагаемого $\dfrac{\partial}{\partial z}\left(u_\varphi n\right)$ вида $$\dfrac{1}{2}(u_{\varphi(i)}n_i+u_{\varphi(i+1)}n_{i+1})-\dfrac{1}{2}|u_{\varphi(i+1/2)}|(n_i-n_{i+1}).$$
...
\bigskip {\bf Схема 8.} Формула для аппроксимации потока: $$\Phi_{i+1/2} = \begin{cases}
...
\bigskip {\bf Схема 7.}  $$\left(\dfrac{\partial n}{\partial t}\right)_i = D_{i+1/2}\dfrac{n_{i+1}-n_i}{h^2} - D_{i-1/2}\dfrac{n_{i}-n_{i-1}}{h^2}+$$ $$+\dfrac{u_{i+1}n_{i+1}-u_{i-1}n_{i-1}}{2h} + $$ $$+\dfrac{1}{h}\left(\dfrac{|u_{\varphi(i+1/2)}|+u_{\varphi(i+1/2)}}{2}\cdot n_{i+1/2}-\dfrac{|u_{\varphi(i-1/2)}|+u_{\varphi(i-1/2)}}{2}\cdot n_{i-1/2}\right)$$
