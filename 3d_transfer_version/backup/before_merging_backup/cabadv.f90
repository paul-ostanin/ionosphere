...
&                                  +((c(i,j,k+1,3,1)*uz(i,j,k+1,1)-c(i,j,k,3,1)*uz(i,j,k,1)))/dlev(k))+0.5*fu(i,j,k)
    end do
  end do
end do
  ...
  do i=1,Nx1
    do j=1,Ny1
      ...
      if (r==-1) then
        ...
        uz(i,j,k,2)=u(i,j,k+1,1)**2 / u(i, j, k+2, 1)
      end if
    end do
  end do
k=Nz
...
&                                  +(c(i,j,k+1,3,2)*uz(i,j,k+1,2)-c(i,j,k,3,2)*uz(i,j,k,2))/dlev(k))+0.5*fu(i,j,k)
    end do
  end do
end do
