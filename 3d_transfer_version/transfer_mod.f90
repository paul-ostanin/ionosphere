...
real, allocatable :: lambda(:), lambda_m(:), phi(:), phi_m(:), cphi_m(:), cphi(:), z_m(:), z(:), t(:), ue(:, :, :), c(:,:,:,:,:), v(:,:,:,:), u(:,:,:,:), fu(:,:,:), ux(:,:,:,:), uy(:,:,:,:), uz(:,:,:,:), uef(:,:,:), kurant(:), dphi(:), dlev(:), dlam(:),u1(:,:),u2(:,:)
...
subroutine init_transfer(z1,arr)
    ...
    allocate(z(1:Nz1))
    allocate(u1(1:Nx1,1:Ny1), u2(1:Nx1,1:Ny1))
    ...
    ! define stationary velocity components
...
!   define boundary conditions
    ...
    phi_curr = phi(j)
        do i = 1, Nx1
                u1(i,j)=f_m(zt,phi_curr)
                u2(i,j)=f_m(zb,phi_curr)
        enddo
    enddo
    ...
    do i = 1, Nx1
        do j = 1, Ny1
            do k = 1, Nz1
                u(i, j, k,1) = arr(Nz-k+1, j, i)
            enddo
        enddo
    enddo
    u(1:Nx1, 1:Ny1, 1:Nz1, 2) = u(1:Nx1, 1:Ny1, 1:Nz1, 1)

    call flux_ini(Nx, Ny, Nz, Nx1, Ny1, Nz1, u(:, :, :, 1), ux(:, :, :, 1), uy(:, :, :, 1), uz(:, :, :, 1), u1, u2, lambda, phi, z, z_m, dlam, dphi, dlev, cphi, pi)

    do i = 1, Nx1
        do j = 1, Ny1
            do k = 1, Nz1
                arr(Nz-k+1, j, i) = u(i, j, k,1)
            enddo
        enddo
    enddo
...
subroutine step_of_transfer( arr_inc,  tau)
    ...
    do n = 1, Nadv
        ...
        call cab_adv(Nx, Ny, Nz, Nx1, Ny1, Nz1, c, u, fu, ux, uy, uz, u1, u2, dlam, dphi, dlev, phi_m, cphi, cphi_m, lambda, lambda_m, z_m, a, pi, dt)
