...
type (vect) rhs_z, rhs_y, z, h, hmid, nO, nO2, nN2, k, p, pcurr, m, n_old, n_new, delta, D, D_node, u, Tn, Ti, Te, Tr, Tp, gradTp, n_day, tau0, n_new_z(1441), n_old_z(1441), n_prev(1441), n_new_y(401), n_old_y(401), n_new_y_1(401), n_old_y_1(401), n_new_z_1(1441), n_old_z_1(1441), error(1441)
...
real (8) tau, tau_1, h0, F_z, delta_norm, eps, tgdelta, sindelta, cosdelta, dphi, phi, coschi, pi, omega, sigma_O2, sigma_N2, sigma_O, sI, cI, R, u_phi, u_phi_1, u_phi_2, u_phi_3, u_phi_4, u_z, u_z_mh, u_z_ph, u_z_m1, u_z_p1, x, A, B, u_phi_ph, u_phi_mh, Ndays, Niter, delta_err, l1_norm, l1_norm_err

!opening files for writing the output
open(unit=10, name='res.txt')
open(unit=12,  name='res_gnp_step2_2day_tau1.txt')
open(unit=110, name='res_gnp_step2_3day_tau2.txt')
open(unit=99, name='res_gnp_err_23.txt')
...
!	call n_z_1(j).init(z.n)
!	call n_z_2(j).init(z.n)
	call n_prev(j).init(z.n)
	call error(j).init(z.n)
	call n_old_z(j).gen()
	call n_old_z_1(j).gen()
end do
...
! angles phi from -90 to 90; conditions in -90 and 90 are set
	...
	if(nonlinear_scheme_type .le. 7) then
        do i = 2, z.n - 1
...
do j = 1, Nphi
	...
	if(nonlinear_scheme_type .le. 7) then
        do i = 2, z.n - 1
		rhs_z.d(i) = n_old_z_1(j).d(i) + tau_1 * p.d(i)
	end do
	...
	if (j .eq. 1 .or. j .eq. Nphi .or. mixed_z_switch .eq. 0) then
		...
		!u_{phi(N-1/2)}
		if(sI .ge. 0) then
			u_phi_ph = -2/R * D.d(i) * sI * cI * (n_old_z_1(j+1).interp(z, m.d(z.n-1))-n_old_z_1(j).interp(z, m.d(z.n-1)))/(n_old_z_1(j+1).interp(z, m.d(z.n-1))+n_old_z_1(j-1).interp(z, m.d(z.n-1)))/dphi
		else
			u_phi_ph = -2/R * D.d(i) * sI * cI * (n_old_z_1(j).interp(z, m.d(z.n-1))-n_old_z_1(j-1).interp(z, m.d(z.n-1)))/(n_old_z_1(j+1).interp(z, m.d(z.n-1))+n_old_z_1(j-1).interp(z, m.d(z.n-1)))/dphi
		end if
		S_z.d(z.n, 1) =    (- D.d(z.n-1)*tau_1/(h.d(z.n-1)**2) + 0.5 * u.d(z.n-1)*tau_1/h.d(z.n-1)) * sI**2 + 0.5*(u_phi_ph+abs(u_phi_ph))*tau_1/h.d(z.n-1)
		S_z.d(z.n, 2) = +1 + (D.d(z.n-1)*tau_1/(h.d(z.n-1)**2) + 0.5 * u.d( z.n )*tau_1/h.d(z.n-1)) * sI**2 + 0.5*(u_phi_ph-abs(u_phi_ph))*tau_1/h.d(z.n-1)
	end if
	...
	if(sI .ge. 0) then
		!u_phi_{i-1/2}		
		u_phi_mh = -2/R * D.d(i) * sI * cI * (n_old_z_1(j+1).interp(z, m.d(i-1))-n_old_z_1(j).interp(z, m.d(i-1)))/(n_old_z_1(j+1).interp(z, m.d(i-1))+n_old_z_1(j-1).interp(z, m.d(i-1)))/dphi
		!u_phi_{i+1/2}
		u_phi_ph = -2/R * D.d(i) * sI * cI * (n_old_z_1(j+1).interp(z, m.d(i))-n_old_z_1(j).interp(z, m.d(i)))/(n_old_z_1(j+1).interp(z, m.d(i))+n_old_z_1(j-1).interp(z, m.d(i)))/dphi
	else
		!u_phi_{i-1/2}		
		u_phi_mh = -2/R * D.d(i) * sI * cI * (n_old_z_1(j).interp(z, m.d(i-1))-n_old_z_1(j-1).interp(z, m.d(i-1)))/(n_old_z_1(j+1).interp(z, m.d(i-1))+n_old_z_1(j-1).interp(z, m.d(i-1)))/dphi
		!u_phi_{i+1/2}
		u_phi_ph = -2/R * D.d(i) * sI * cI * (n_old_z_1(j).interp(z, m.d(i))-n_old_z_1(j-1).interp(z, m.d(i)))/(n_old_z_1(j+1).interp(z, m.d(i))+n_old_z_1(j-1).interp(z, m.d(i)))/dphi
	end if

		S_z.d(i, 1) = (-D.d(i-1)*tau_1/(hmid.d(i) * h.d(i-1)) + u.d(i-1)*tau_1/(h.d(i) + h.d(i-1))) * sI**2 + (u_phi_mh+abs(u_phi_mh))*tau_1/(2*h0)
		S_z.d(i, 2) = 1 + k.d(i)*tau_1 + (D.d(i-1)/h.d(i-1) + D.d(i)/h.d(i)) * tau_1 / hmid.d(i) * sI**2 +(u_phi_mh-abs(u_phi_mh))*tau_1/(2*h0)-(u_phi_ph+abs(u_phi_ph))*tau_1/(2*h0)
		S_z.d(i, 3) = (-D.d( i )*tau_1/(hmid.d(i) * h.d( i )) - u.d(i+1)*tau_1/(h.d(i) + h.d(i-1))) * sI**2 - (u_phi_ph-abs(u_phi_ph))*tau_1/(2*h0)
...
!do i = 2, z.n-1
!do j = 2, Nphi
!	n_new_y_1(i).d(j) = n_old_y_1(i).d(j)
!end do
!end do
...
do i = 2, z.n-1
	...
	if(B(phi) .le. 0) then
		...
		S_y.d(j, 1) =  (-D_node.d(i)/R * A(phi-dphi/2)/(dphi**2) + (-u.d(i)/2)*B(phi-dphi)/(2*dphi) + &
				0.5*D_node.d(i)/2*B(phi-dphi)/h0/dphi)*tau_1/(R)/cos(phi)
		S_y.d(j, 2) = 1+(D_node.d(i)/R *(A(phi-dphi/2) + A(phi+dphi/2))/(dphi**2) - &
				    D_node.d(i)/2*B(phi)/h0/dphi) * tau_1/(R)/cos(phi)
		S_y.d(j, 3) =  (-D_node.d(i)/R * A(phi+dphi/2)/(dphi**2) - (-u.d(i)/2)*B(phi+dphi)/(2*dphi) + &
				0.5*D_node.d(i)/2*B(phi+dphi)/h0/dphi)*tau_1/(R)/cos(phi)

		rhs_y.d(j) = n_old_y_1(i).d(j)-&
				0.5*tau_1*D_node.d(i)/(2*R*cos(phi)*h0*dphi)*(B(phi)*(n_old_y_1(i-1).d(j)+n_old_y_1(i+1).d(j)) - &
									    B(phi+dphi)*n_old_y_1(i-1).d(j+1) - &
									    B(phi-dphi)*n_old_y_1(i+1).d(j-1))
	...
	phi = (j-0.5)*dphi-pi/2
		...
		S_y.d(j, 2) = 1+(D_node.d(i)/R *A(phi+dphi/2)/(dphi**2) - &
				    D_node.d(i)/2*B(phi)/h0/dphi + (-u.d(i)/2)*B(phi-dphi)/(2*dphi)+ &
					0.5*D_node.d(i)/2*B(phi)/h0/dphi) * tau_1/(R)/cos(phi)
		S_y.d(j, 3) =  (-D_node.d(i)/R * A(phi+dphi/2)/(dphi**2) - (-u.d(i)/2)*B(phi+dphi)/(2*dphi) + &
				0.5*D_node.d(i)/2*B(phi+dphi)/h0/dphi)*tau_1/(R)/cos(phi)
...
!	if (t*tau .eq. 86400*1 ) then
!		do j = 1, Nphi
!		do i = 1, z.n
!			write(12,*) (j-5E-1)*180/(Nphi)-90, 100+400/(z.n-1)*(i-1), n_new_z(j).d(i)
!			n_z_1(j).d(i) = n_new_z(j).d(i)
!
!		end do
!		write (12, *)
!		end do
...
!		end do
!	if (t*tau .eq. 86400*0.5 ) then
!		do j = 1, Nphi
!		do i = 1, z.n
!			write(110,*) (j-5E-1)*180/(Nphi)-90, 100+400/(z.n-1)*(i-1), n_new_z(j).d(i)
!			n_z_2(j).d(i) = n_new_z(j).d(i)
!
!		end do
!		write (110, *)
!		end do
...
!do i = 1, z.n!
!	do j = 1, Nphi
!		delta_err = delta_err + abs(n_z_1(j).d(i) - n_z_2(j).d(i))
!		error(j).d(i) = n_z_2(j).d(i)-n_z_1(j).d(i)
!	end do
!end do
!
!		do j = 1, Nphi
!		do i = 1, z.n
!			write(99,'(F42.15,1X,F42.15,1X,F42.15)') (j-5E-1)*180/(Nphi)-90, real(100+400/(z.n-1)*(i-1),8), error(j).d(i)
!			
!
!		end do
!		write (99, *)
