...
if(j .le. 0) then!86400/tau*2) then
...
if(diurnal_on .eq. 1) then
	...
	day = (j*tau)/86400 - 2 !starting from the night, to change for the day take this code:!+ 1/2 !starting from the middle of the 1-st day
	...
	cosdelta = sqrt(1-sindelta**2) !cos of the zenith angle is > 0
        ...
        do i = 2, z.n - 1
		coschi = sin(-pi/2)*sindelta
		if(coschi > 1E-6) then
		        b.d(i) = nold(1).d(i) + tau * p.d(i) * exp(tau0.d(i) * (1-1/coschi))
        	else
			b.d(i) = nold(1).d(i)
        	end if
	end do

	b.d(z.n) = tau/h.d(z.n-1) * Fub + nold(1).d(z.n)

	if(coschi > 1E-6) then
	        b.d(1) = p.d(1)/k.d(1) * exp(tau0.d(i) * (1-1/coschi))
       	else
		b.d(1) = 0
       	end if

	nold( 1) = tridiagonal_matrix_algorithm(S, b)


        do i = 2, z.n - 1
		coschi = sin(+pi/2)*sindelta
		if(coschi > 1E-6) then
		        b.d(i) = nold(Nphi).d(i) + tau * p.d(i) * exp(tau0.d(i) * (1-1/coschi))
        	else
			b.d(i) = nold(Nphi).d(i) 
        	end if
	end do

	b.d(z.n) = tau/h.d(z.n-1) * Fub + nold(Nphi).d(z.n)
	...
	nold(Nphi) = tridiagonal_matrix_algorithm(S, b)
...
if(diurnal_on .eq. 1) then
	...
	day = (j*tau)/86400! - 2!+ 1/2 !starting from the middle of the 1-st day
	tgdelta = tan(pi/180*23.5) * sin(2*pi/365 * (day - 80))
	sindelta = tgdelta/sqrt(1+tgdelta**2)
	cosdelta = sqrt(1-sindelta**2) !cos of the zenith angle is > 0

        do i = 2, z.n - 1
		coschi = sin(-pi/2+(q-1)*dphi)*sindelta - cos(-pi/2+(q-1)*dphi)*cosdelta*cos(omega * (tau * j )) !+ 86400/2 - 2 - start: middle of the 1 day
		if(coschi > 1E-6) then
		...
		if(j .eq. 86400/tau * 2) then
		print *, j*tau
		print *, q-1
		do i = 1, z.n
			write(12,*) q-1, 100+400/(z.n-1)*(i-1), exp(tau0.d(i) * (1-1/coschi))
		end do
		write (12, *)
		end if
...
else if(diurnal_on .eq. 0 .and. nonlinear_scheme_type .ne. 8) then
...
!		S.d(z.n, 2) = +1 + (D.d(z.n-1)*tau/(h.d(z.n-1)**2) + 0.5 * u.d( z.n )*tau/h.d(z.n-1)) * sI**2 + 0.5*(u_phi_ph-abs(u_phi_ph))*tau/h.d(z.n-1)
		...
		S.d(z.n, 2) = +1 + (D.d(z.n-1)*tau/(h.d(z.n-1)**2) + 0.5 * u.d( z.n )*tau/h.d(z.n-1)) * sI**2 + &
				    0.5*D_node.d(i)*tau*sI*cI/(R*h0*dphi)

		b.d(z.n) = +tau/h.d(z.n-1) * Fub + nold(q).d(z.n) - &
				0.5*tau*sI*cI/(2*R*h0*dphi)*(-D_node.d(z.n)*nold(q-1).d(z.n) + D_node.d(i-1)*nold(q-1).d(z.n-1))
