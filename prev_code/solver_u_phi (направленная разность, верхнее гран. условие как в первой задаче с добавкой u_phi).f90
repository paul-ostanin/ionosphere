...
real (8) tau, h0, Fub, delta_norm, eps, tgdelta, sindelta, cosdelta, dphi, phi, coschi, pi, omega, sigma_O2, sigma_N2, sigma_O, sI, cI, R, u_phi
...
! angles phi from -90 to 90; conditions in -90 and 90 are set, the step is dphi = 2
	...
	if (nold(q+1).d(z.n) .ne. 0 .and. nold(q-1).d(z.n) .ne. 0 .and. nold(q+1).d(z.n-1) .ne. 0 .and. nold(q+1).d(z.n-1) .ne. 0) then
		...
		S.d(z.n, 1) =    (- D.d(z.n-1)*tau/(h.d(z.n-1)**2) + 0.5 * (u.d(z.n-1) + u_phi)*tau/h.d(z.n-1)) * sI**2
		u_phi = -1/R * D.d(i) * sI * cI * log(nold(q+1).d(z.n)/nold(q-1).d(z.n))/(2*dphi)
		S.d(z.n, 2) = -1 + (D.d(z.n-1)*tau/(h.d(z.n-1)**2) + 0.5 * (u.d( z.n ) + u_phi)*tau/h.d(z.n-1)) * sI**2
	else
