...
type (vect) rhs_z, rhs_y, z, h, hmid, nO, nO2, nN2, k, p, pcurr, m, n_old, n_new, delta, D, D_node, u, Tn, Ti, Te, Tr, Tp, gradTp, n_day, tau0, n_new_z(1441), n_old_z(1441), n_prev(1441), n_new_y(401), n_old_y(401), n_z_1(1441), n_z_2(1441), error(1441)
...
real (8) tau, h0, F_z, delta_norm, eps, tgdelta, sindelta, cosdelta, dphi, phi, coschi, pi, omega, sigma_O2, sigma_N2, sigma_O, sI, cI, R, u_phi, u_phi_1, u_phi_2, u_phi_3, u_phi_4, u_z, u_z_mh, u_z_ph, u_z_m1, u_z_p1, x, A, B, u_phi_ph, u_phi_mh, Ndays, Niter, delta_err

!opening files for writing the output
open(unit=101, name='res_1.txt')
open(unit=121, name='res_gnp_step2_diurnal_1.txt')
open(unit=10140, name='res_140.txt')
open(unit=12140, name='res_gnp_step2_diurnal_140.txt')
open(unit=10150, name='res_150.txt')
open(unit=12150, name='res_gnp_step2_diurnal_150.txt')
open(unit=10160, name='res_160.txt')
open(unit=12160, name='res_gnp_step2_diurnal_160.txt')
open(unit=10170, name='res_170.txt')
open(unit=12170, name='res_gnp_step2_diurnal_170.txt')
open(unit=10180, name='res_180.txt')
open(unit=12180, name='res_gnp_step2_diurnal_180.txt')
...
! angles phi from -90 to 90; conditions in -90 and 90 are set
	...
	coschi = sin(-pi/2+(j-0.5)*dphi)*sindelta - cos(-pi/2+(j-0.5)*dphi)*cosdelta*cos(omega * (tau * t + 86400/2)) !start: middle of the 1 day
...
!output block
	...
	if (t .eq. 140) then
		do j = 1, Nphi
		do i = 1, z.n
			write(12140,*) (j-5E-1)*180/(Nphi)-90, 100+400/(z.n-1)*(i-1), n_new_z(j).d(i)
!			n_z_1(j).d(i) = n_new_z(j).d(i)

		end do
		write (12140, *)
		call n_new_z(j).print(10140)
		end do
	end if
	if (t .eq. 150) then
		do j = 1, Nphi
		do i = 1, z.n
			write(12150,*) (j-5E-1)*180/(Nphi)-90, 100+400/(z.n-1)*(i-1), n_new_z(j).d(i)
!			n_z_1(j).d(i) = n_new_z(j).d(i)

		end do
		write (12150, *)
		call n_new_z(j).print(10150)
		end do
	end if
	if (t .eq. 160) then
		do j = 1, Nphi
		do i = 1, z.n
			write(12160,*) (j-5E-1)*180/(Nphi)-90, 100+400/(z.n-1)*(i-1), n_new_z(j).d(i)
!			n_z_1(j).d(i) = n_new_z(j).d(i)

		end do
		write (12160, *)
		call n_new_z(j).print(10160)
		end do
	end if
	...
	if (t .eq. 170) then
		do j = 1, Nphi
		do i = 1, z.n
			write(12170,*) (j-5E-1)*180/(Nphi)-90, 100+400/(z.n-1)*(i-1), n_new_z(j).d(i)
!			n_z_1(j).d(i) = n_new_z(j).d(i)

		end do
		write (12170, *)
		call n_new_z(j).print(10170)
		end do
	end if
	...
	if (t .eq. 180) then
		do j = 1, Nphi
		do i = 1, z.n
			write(12180,*) (j-5E-1)*180/(Nphi)-90, 100+400/(z.n-1)*(i-1), n_new_z(j).d(i)
!			n_z_1(j).d(i) = n_new_z(j).d(i)

		end do
		write (12180, *)
		call n_new_z(j).print(10180)
		end do
	end if
