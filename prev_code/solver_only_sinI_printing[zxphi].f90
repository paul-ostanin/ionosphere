...
type (vect) b, z, h, hmid, nO, nO2, nN2, k, p, pcurr, m, njold, njnew, delta, D, u, Tn, Ti, Te, Tr, Tp, gradTp, nday, tau0, n(50)
integer i, j, out_file, scheme_type, Te0, Tn0, Ti0, day
real (8) tau, h0, Fub, delta_norm, eps, tgdelta, sindelta, cosdelta, phi, coschi, pi, omega, sigma_O2, sigma_N2, sigma_O, sI, Nphi, dphi
...
sI = sin(atan(2*tan(-pi/2+(j-1)*dphi)))
...
S.d(z.n, 1) = -D.d(z.n - 1) * (sI**2)/h0**2
S.d(z.n, 2) = D.d(z.n - 1) * (sI**2)/h0**2 +  u.d(z.n)/h0

else if (scheme_type == 2) then
!new upper boundary condition:
S.d(z.n, 1) =    - D.d(z.n-1) * (sI**2)*tau/(h.d(z.n-1)**2) + 0.5 * u.d(z.n-1) * (sI**2)*tau/h.d(z.n-1)
S.d(z.n, 2) = +1 + D.d(z.n-1) * (sI**2)*tau/(h.d(z.n-1)**2) + 0.5 * u.d( z.n ) * (sI**2)*tau/h.d(z.n-1)
end if
...
! old scheme
	S.d(i, 1) = -D.d(i-1)*(sI**2)*tau/h0**2
	S.d(i, 2) = (1 + tau * k.d(i)) + u.d(i) * (sI**2)*tau/h0 + (D.d(i-1) + D.d(i)) * (sI**2)*tau/h0**2
	S.d(i, 3) = -D.d( i ) * (sI**2)*tau/h0**2 - u.d(i+1)*(sI**2)*tau/h0

else if (scheme_type == 1 .OR. scheme_type == 2) then
! symmetric scheme
	S.d(i, 1) = -D.d(i-1) * (sI**2)*tau/(hmid.d(i) * h.d(i-1)) + u.d(i-1) * (sI**2)*tau/(h.d(i) + h.d(i-1))
	S.d(i, 2) = 1 + k.d(i)*tau + (D.d(i-1)/h.d(i-1) + D.d(i)/h.d(i)) * (sI**2) * tau / hmid.d(i)
	S.d(i, 3) = -D.d( i ) * (sI**2)*tau/(hmid.d(i) * h.d( i )) - u.d(i+1) * (sI**2)*tau/(h.d(i) + h.d(i-1))
end if
