...
real (8) tau, tau_1, h0, F_z, delta_norm, eps, tgdelta, sindelta, cosdelta, dphi, phi, coschi, pi, omega, sigma_O2, sigma_N2, sigma_O, sI, cI, R, u_phi, u_phi_1, u_phi_2, u_phi_3, u_phi_4, u_z, u_z_mh, u_z_ph, u_z_m1, u_z_p1, x, A, B, u_phi_ph, u_phi_mh, Ndays, Niter, delta_err, l1_norm
...
! angles phi from -90 to 90; conditions in -90 and 90 are set
	...
	if (j .eq. 1 .or. j .eq. Nphi .or. mixed_z_switch .eq. 0) then
		...
		S_z.d(z.n, 1) =    (- D.d(z.n-1)*tau/(h.d(z.n-1)**2) + 0.5 * u.d(z.n-1)*tau/h.d(z.n-1)) * sI**2 - D_node.d(i-1)*sI*cI/(2*R*h0*dphi)*tau
		S_z.d(z.n, 2) = +1 + (D.d(z.n-1)*tau/(h.d(z.n-1)**2) + 0.5 * u.d( z.n )*tau/h.d(z.n-1)) * sI**2 + D_node.d(i)*sI*cI/(2*R*h0*dphi)*tau
 

		rhs_z.d(z.n) = +tau/h.d(z.n-1) * F_z + n_old_z(j).d(z.n) - &
				tau*sI*cI/(2*R*h0*dphi)*(-D_node.d( i )*n_old_z(j+1).d( i ) + D_node.d(i-1)*n_old_z(j-1).d(i-1))
	...
	if(sI .ge. 0) then
		...
		S_z.d(i, 1) =  (-D.d(i-1)*tau/(hmid.d(i) * h.d(i-1)) + u.d(i-1)*tau/(h.d(i) + h.d(i-1))) * sI**2 - &
				D_node.d(i-1)*sI*cI/(2*R*h0*dphi)*tau
		S_z.d(i, 2) = 1 + k.d(i)*tau + (D.d(i-1)/h.d(i-1) + D.d(i)/h.d(i)) * tau / hmid.d(i) * sI**2 + &
				    D_node.d(i)*sI*cI/(R*h0*dphi)*tau
		S_z.d(i, 3) =  (-D.d( i )*tau/(hmid.d(i) * h.d( i )) - u.d(i+1)*tau/(h.d(i) + h.d(i-1))) * sI**2 - &
				D_node.d(i+1)*sI*cI/(2*R*h0*dphi)*tau

		rhs_z.d(i) = n_old_z(j).d(i) + tau * p.d(i) - &
				tau*sI*cI/(2*R*h0*dphi)*(-D_node.d( i )*n_old_z(j-1).d( i ) - D_node.d( i )*n_old_z(j+1).d( i ) + &
							  D_node.d(i-1)*n_old_z(j-1).d(i-1) + D_node.d(i+1)*n_old_z(j+1).d(i+1))
...
do i = 1, z.n
	do j = 1, Nphi
		delta_err = delta_err + abs(n_old_z(j).d(i) - n_prev(j).d(i))
		l1_norm = l1_norm + abs(n_old_z(j).d(i))
	end do
end do
