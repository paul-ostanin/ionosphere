...
integer i, j, t, Te0, Tn0, Ti0, day, nonlinear_scheme_type, diurnal_on, Nphi, pk_switch, mixed_z_switch, mixed_y_switch, transf_yz_switch, transf_y_switch, second_step_scheme_type, upper_bound_type, monotonizator
...
! angles phi from -90 to 90; conditions in -90 and 90 are set
	...
	!integrated upper boundary condition
		...
		rhs_z.d(z.n) = +tau/h.d(z.n-1) * F_z + n_old_z(j).d(z.n) - 0.25*tau*sI*cI/(R*h0*dphi)*D.d(z.n-1)* &
				(n_old_z(j+1).d(z.n) + n_old_z(j+1).d(z.n-1) - n_old_z(j-1).d(z.n) - n_old_z(j-1).d(z.n-1))
	...
	if(sI*cI .ge. 0) then
		...
		rhs_z.d(i) = n_old_z(j).d(i) + tau * p.d(i) - &
				0.5*tau*sI*cI/(2*R*h0*dphi)*(-D_node.d(i)*(n_old_z(j-1).d(i)+n_old_z(j+1).d(i)) + &
									     D_node.d(i-1)*n_old_z(j-1).d(i-1) + &
									     D_node.d(i+1)*n_old_z(j+1).d(i+1))
...
do i = 2, z.n-1
	...
	if(B(phi) .le. 0) then
		...
		S_y.d(j, 1) =  (-D_node.d(i)/R * A(phi-dphi/2)/(dphi**2) + (-u.d(i)/2)*B(phi-dphi)/(2*dphi) + &
				0.5*mixed_y_switch*D_node.d(i)/2*B(phi-dphi)/h0/dphi)*tau/(R)/cos(phi)
		S_y.d(j, 2) = 1+(D_node.d(i)/R *(A(phi-dphi/2) + A(phi+dphi/2))/(dphi**2) - &
				    mixed_y_switch*D_node.d(i)/2*B(phi)/h0/dphi) * tau/(R)/cos(phi)
		S_y.d(j, 3) =  (-D_node.d(i)/R * A(phi+dphi/2)/(dphi**2) - (-u.d(i)/2)*B(phi+dphi)/(2*dphi) + &
				0.5*mixed_y_switch*D_node.d(i)/2*B(phi+dphi)/h0/dphi)*tau/(R)/cos(phi)

		rhs_y.d(j) = n_old_y(i).d(j)-&
				0.5*mixed_y_switch*tau*D_node.d(i)/(2*R*cos(phi)*h0*dphi)*(B(phi)*(n_old_y(i-1).d(j)+n_old_y(i+1).d(j)) - &
	...
	phi = (j-0.5)*dphi-pi/2
		...
		S_y.d(j, 2) = 1+(D_node.d(i)/R *A(phi+dphi/2)/(dphi**2) - &
				    mixed_y_switch*D_node.d(i)/2*B(phi)/h0/dphi + (-u.d(i)/2)*B(phi-dphi)/(2*dphi)+ &
					0.5*mixed_y_switch*D_node.d(i)/2*B(phi)/h0/dphi) * tau/(R)/cos(phi)
		S_y.d(j, 3) =  (-D_node.d(i)/R * A(phi+dphi/2)/(dphi**2) - (-u.d(i)/2)*B(phi+dphi)/(2*dphi) + &
				0.5*mixed_y_switch*D_node.d(i)/2*B(phi+dphi)/h0/dphi)*tau/(R)/cos(phi)
	...
	phi = (j-0.5)*dphi-pi/2
		...
		S_y.d(j, 1) =  (-D_node.d(i)/R * A(phi-dphi/2)/(dphi**2) + (-u.d(i)/2)*B(phi-dphi)/(2*dphi)- &
				0.5*mixed_y_switch*D_node.d(i)/2*B(phi-dphi)/h0/dphi)*tau/(R)/cos(phi)
		S_y.d(j, 2) = 1+(D_node.d(i)/R *A(phi-dphi/2)/(dphi**2) + &
				    mixed_y_switch*D_node.d(i)/2*B(phi)/h0/dphi - (-u.d(i)/2)*B(phi)/(2*dphi) - &
					0.5*mixed_y_switch*D_node.d(i)/2*B(phi)/h0/dphi) * tau/(R)/cos(phi)
		S_y.d(j, 3) =  0
...
do i = 1, z.n
	...
	if (t .eq. 1500) then
		do j = 1, Nphi
		do i = 1, z.n
			write(12,*) (j-5E-1)*180/(Nphi)-90, 100+400/(z.n-1)*(i-1), n_old_z(j).d(i)
		end do
		write (12, *)
		end do
		do j = 1, Nphi
