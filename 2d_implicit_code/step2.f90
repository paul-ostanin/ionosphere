...
call dcopy(matrix_size, 0d0, 0, n, 1)
call dcopy(matrix_size, 0d0, 0, prev_n, 1)
    ...
    do i = 1, Nz
        do j = 1, Nphi
            ...
            if (i == 1) then
                ...
                operator(-1, :) = [0d0, (-D.d(i-1)*tau/(hmid.d(i) * h.d(i-1)) + u.d(i-1)*tau/(h.d(i) + h.d(i-1))) * sI**2, 0d0]
                operator( 0, :) = [0d0, 1 + k.d(i)*tau + (D.d(i-1)/h.d(i-1) + D.d(i)/h.d(i)) * tau / hmid.d(i) * sI**2   , 0d0]
                operator(+1, :) = [0d0, (-D.d( i )*tau/(hmid.d(i) * h.d( i )) - u.d(i+1)*tau/(h.d(i) + h.d(i-1))) * sI**2, 0d0]

            else if ((j == 1 .or. j == Nphi) .and. i == Nz) then

                !left and right upper corners
                operator(-1, :) = [0d0,   (- D.d(z.n-1)*tau/(h.d(z.n-1)**2) + 0.5 * u.d(z.n-1)*tau/h.d(z.n-1)) * sI**2, 0d0]
                operator( 0, :) = [0d0, 1 + (D.d(z.n-1)*tau/(h.d(z.n-1)**2) + 0.5 * u.d( z.n )*tau/h.d(z.n-1)) * sI**2, 0d0]
                operator(+1, :) = [0d0, 0d0, 0d0]
                ...
                if(sI*cI .ge. 0) then
                    mixed_z(-1, :) = [ 0.5*D_node.d(i-1)*sImh*cImh*tau/(R*h0*dphi), -0.5*D_node.d(i-1)*sImh*cImh*tau/(R*h0*dphi)            ,  0d0                                        ]
                    mixed_z( 0, :) = [-0.5*D_node.d( i )*sImh*cImh*tau/(R*h0*dphi),  0.5*D_node.d( i )*(sIph*cIph+sImh*cImh)*tau/(R*h0*dphi), -0.5*D_node.d( i )*sIph*cIph*tau/(R*h0*dphi)]
                    mixed_z(+1, :) = [ 0d0,                                         -0.5*D_node.d(i+1)*sIph*cIph*tau/(R*h0*dphi)            ,  0.5*D_node.d(i+1)*sIph*cIph*tau/(R*h0*dphi)]
                    ...
                    mixed_z(-1, :) = [ 0d0,                                          0.5*D_node.d(i-1)*sIph*cIph*tau/(R*h0*dphi)          , -0.5*D_node.d(i-1)*sIph*cIph*tau/(R*h0*dphi)]
                    mixed_z( 0, :) = [ 0.5*D_node.d( i )*sImh*cImh*tau/(R*h0*dphi), -0.5*D_node.d(i)*(sImh*cImh+sIph*cIph)*tau/(R*h0*dphi),  0.5*D_node.d( i )*sIph*cIph*tau/(R*h0*dphi)]
                    mixed_z(+1, :) = [-0.5*D_node.d(i+1)*sImh*cImh*tau/(R*h0*dphi),  0.5*D_node.d(i+1)*sImh*cImh*tau/(R*h0*dphi)          ,  0d0                                        ]
