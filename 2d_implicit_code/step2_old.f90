...
real*8 arr(max_nonzero), f(max_size), n(max_size), prev_n(max_size), rw(maxwr), v(max_size)
...
integer sz, counter_a, counter_ia, counter_rhs
...
real*8 stencil(Nz, Nphi, -1:1, -1:1), foobar(-1:1, -1:1)
...
!iterative solving of the matrix
                ...
                do s_i = -1, 1
                    do s_j = -1, 1
                        if (i + s_i >= 1 .and. i + s_i <= Nz .and. j + s_j >= 1 .and. j + s_j <= Nphi) then
                            stencil(i, j, s_i, s_j) = foobar(s_i, s_j)
                        endif
                    enddo
                enddo
                rhs_z_phi(i, j) = sin(i * pi / Nz) * sin(j * pi / Nphi)
            end do 
        end do
 ...
 !       currt = t*tau
                ...
                do s_j = -1, 1
                    ...
                    do s_i = -1, 1
                        if (stencil(i, j, s_i, s_j) /= 0) then
                            ...
                            ja(counter_a) = (j + s_j - 1) * sz + (i + s_i - 1) + 1
        ...
        !call dcopy(matrix_bt(matrix_size, f, 1, f, 1))
