    ...
    do i = 1, Nz
        do j = 1, Nphi
            ...
            if (i == 1) then
                ...
                diffusion_transfer_z( 0, :) = [0d0, D.d(i-1)*tau/(h0**2)*sI**2 + u.d( i )*tau/(2*h0)*sI**2, 0d0] 
                ...
                if(j == 1 .or. j == Nphi) then
                    if(sI*cI .ge. 0) then
                        ...
                        mixed_z(-1, :) = [ 0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi), 0d0]
                    else
                        mixed_z(+1, :) = [ 0d0,  0d0                                    ,  0d0                                    ]
                        mixed_z( 0, :) = [ 0d0,  0d0                                    ,  0d0                                    ]
                        mixed_z(-1, :) = [ 0d0,  0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi)]
                    end if
                else
                    if(sI*cI .ge. 0) then
                        mixed_z(+1, :) = [ 0d0                                    ,  0d0                                    ,  0d0                                    ]
                        mixed_z( 0, :) = [ 0d0                                    ,  0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi)]
                        mixed_z(-1, :) = [ 0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi),  0d0                                    ]
                    else
                        mixed_z(+1, :) = [ 0d0                                    ,  0d0                                    ,  0d0                                    ]
                        mixed_z( 0, :) = [ 0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi),  0d0                                    ]
                        mixed_z(-1, :) = [ 0d0,                                      0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sI*cI/(R*h0*dphi)]
                    end if
                end if
