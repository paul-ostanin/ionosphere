...
integer printing_mode, s_i, s_j, i, j, t, Te0, Tn0, Ti0, day, nonlinear_scheme_type, profile_output, diurnal_on, convergence_test, pk_switch, mixed_z_switch, mixed_y_switch, transf_yz_switch, transf_y_switch, second_step_scheme_type, upper_bound_type, monotonizator
...
do j = 1, Nphi
    ...
    do i = 1, z.n     
        p_mod(j).d(i) = k.d(i)*f_m(z.d(i), phi) - ( &
                        ...
                        +0)
        !write(700,*) (j-5E-1)*180/(Nphi)-90, 100+(Hmax - 100)/(z.n-1)*(i-1), p_mod(j).d(i)
    end do
    write(700, *) (j-5E-1)*180/(Nphi)-90, F_ub.d(j)*tau/h0
        ...
        do j = 1, Nphi
            ...
            if (i == 1) then
                ...
                if(j == 1 .or. j == Nphi) then
                    ...
                    if(sI*cI .ge. 0) then
                        mixed_z(+1, :) = [ 0d0                                        ,  0d0                                        ,                                          0d0]
                        mixed_z( 0, :) = [ 0d0                                        ,  0.5*D_node.d(i-1)*tau*sIph*cIph/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sIph*cIph/(R*h0*dphi)]
                        mixed_z(-1, :) = [ 0.5*D_node.d(i-1)*tau*sImh*cImh/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sImh*cImh/(R*h0*dphi),  0d0                                        ]
                    else
                        mixed_z(+1, :) = [ 0d0                                        ,  0d0                                        ,  0d0                                        ]
                        mixed_z( 0, :) = [ 0.5*D_node.d(i-1)*tau*sImh*cImh/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sImh*cImh/(R*h0*dphi),  0d0                                        ]
                        mixed_z(-1, :) = [ 0d0                                        ,  0.5*D_node.d(i-1)*tau*sIph*cIph/(R*h0*dphi), -0.5*D_node.d(i-1)*tau*sIph*cIph/(R*h0*dphi)]
                    end if
                end if
